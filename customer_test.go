...
func TestCreateCustomerToken(t *testing.T) {
	...
	if string(resp.Cards[0].Name) != string(token.Card.Name) {
		t.Errorf("Expected Card Name %s, got %v", token.Card.Name, resp.Cards[0].Name)
	}
}
...
func TestRetrieveCustomer(t *testing.T) {
	...
	if string(cust.Cards[0].Name) != cust2.Card.Name {
		t.Errorf("Expected Card Name %s, got %s", cust2.Card.Name, cust.Cards[0].Name)
	}
	if cust.Cards[0].Last4 != "4242" {
		t.Errorf("Expected Card Last4 %d, got %d", "4242", cust.Cards[0].Last4)
	}
	if cust.Cards[0].ExpYear != cust2.Card.ExpYear {
		t.Errorf("Expected Card ExpYear %d, got %d", cust2.Card.ExpYear, cust.Cards[0].ExpYear)
	}
	if cust.Cards[0].ExpMonth != cust2.Card.ExpMonth {
		t.Errorf("Expected Card ExpMonth %d, got %d", cust2.Card.ExpMonth, cust.Cards[0].ExpMonth)
	}
	if cust.Cards[0].Type != Visa {
		t.Errorf("Expected Card Type %s, got %s", Visa, cust.Cards[0].Type)
	}
}
